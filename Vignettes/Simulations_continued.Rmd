...
estimate_B_ls <- function(Theta, P){
  ppt <- P %*% t(P)
  # Use pseudoinverse if matrix is singular (more robust)
...
fit_G_given_p0 <- function(Theta, B, times, p0, starts = 40, N_pseudo = 3000, seed_base = 1L){
  ...
  for(s in 1:starts){
    ...
    init <- runif(4, 0.01, 0.8)  # Direct rates instead of log-rates
...
rates_true <- c(muE=0.20, muM=0.18, lamE=0.25, lamM=0.22)  # Direct rates
G_true   <- build_G(rates_true)
